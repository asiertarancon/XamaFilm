...
namespace XamaFilm.ViewModels
{
    public class DetailSeasonSerieViewModel : Base.ViewModelBase
    {
        ...
        private async void OnItemSelected(SeasonEpisode seasonEpisode)
        {
            await ViewModelLocator.Instance.Resolve<INavigationService>().NavigateToAsync<DetailEpisodeViewModel>(seasonEpisode);
        }
    }
}
