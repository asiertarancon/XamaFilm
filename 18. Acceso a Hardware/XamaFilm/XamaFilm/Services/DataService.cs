...
namespace XamaFilm.Services
{
    public class DataService : IDataService
    {
        ...
        public async Task<Movie> GetMovieByIdAsync(string id)
        {
            //http://www.omdbapi.com/?apikey=xxx&i=tt0944947
            var apiUri = $"{API}&i={id}";
            var movie = await _requestService.GetAsync<Movie>(apiUri);
            var position = GetRandomLocation();
            movie.Latitude = position.Item1;
            movie.Longitude = position.Item2;
            return movie;
        }
        ...
        private Tuple<double,double> GetRandomLocation()
        {
